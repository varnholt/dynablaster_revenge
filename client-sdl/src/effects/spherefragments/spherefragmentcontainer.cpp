      ...
      mLavaMapParam(-1)
{
   ...
   // init shader
   mShader = activeDevice->loadShader("spherefragments-vert.glsl", "spherefragments-frag.glsl");
...
void SphereFragmentContainer::drawFragments(const Vector& camPos)
{
   ...
   activeDevice->setParameter(mLightParam, lightPosition);

   activeDevice->setParameter(mCameraParam, camPos);

   activeDevice->setParameter(mProjectMatrixParam, proj);
   ...
   for (int i = 0; i < mFragments.size(); i++)
   {
      ...
      activeDevice->setParameter(mModelMatrixParam, fragment->getMatrices(), count);

      activeDevice->setParameter(mFresnelParam, fragment->getFresnelFactors(), count);
